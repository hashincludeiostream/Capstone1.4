...
export const CustomerAuthPage: React.FC<CustomerAuthPageProps> = ({
  ...
  const handleGoogleAuth = async () => {
    ...
    if (email && email.includes('@')) {
      ...
      // Open the interactive Google Account Chooser
        ...
        <div className="lg:col-span-7 p-6 sm:p-10 flex flex-col justify-center">
          ...
          <div className="mt-5">
            <button
              ...
              className="w-full py-2.5 px-4 bg-white hover:bg-pink-50/50 border-2 border-pink-200 hover:border-pink-300 text-gray-800 font-semibold rounded-xl text-xs sm:text-sm shadow-xs flex items-center justify-center space-x-2.5 transition-all cursor-pointer disabled:opacity-50"
            >
              ...
              <span>{googleLoading ? 'Connecting & Verifying Account...' : 'Continue with Google / Gmail Account'}</span>
            </button>
            <div className="relative my-4">
              <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-pink-100"></div></div>
              <div className="relative flex justify-center text-[11px] uppercase tracking-wider"><span className="bg-white px-2.5 text-gray-400 font-medium">Or continue with email</span></div>
            </div>
          </div>
